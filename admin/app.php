...
if(isset($_GET['maxdate'])){
  $maxdate=$_GET['date'];
  $query="UPDATE details set maxdate='$maxdate'";
  mysqli_query($conn,$query);
  header("REFRESH:0;url=app.php");
}
//echo $maxdate;
...
<div class="sidebar-wrapper">
  ...
  <li class="mazen2">
        ...
        <ul class="mazen" style="display:none">
          ...
          <li><a href="app.php" class="list h active">المواعيد</a></li>
    ...
    <div class="main-panel" style="height: 400vh;text-align: right;width:82% !important;">
              ...
              <div class="input-group no-border">
                ...
                <div class="input-group-append">
                  <div class="input-group-text">
                    <i class="nc-icon nc-zoom-split" onclick="searchdate()" style=""></i>
                  </div>
                </div>
              </div>
            </form>
      ...
      <!-- End Navbar -->
      <div class="content" >
        <form class="maxdate"action="<?php echo $_SERVER['PHP_SELF'];?>">
        <input type="date" value="<?php echo $maxdate;?>"name="date" class="form-control" style="width:19vw;display:unset"  >
        <input type="submit" style="margin-bottom:1.1vw" name="maxdate"class="btn submitmax" value="change Max-Date ( <?php echo $maxdate;?> )">
        <a href="copy.php"  rel="noopener noreferrer"><input type="button" style="margin-bottom:1.1vw" class="btn submitmax" value="Go to appointment backups">
        </a>
        <input type="submit" style="margin-bottom:1.1vw" name="deleteall" class="btn submitmax" value="Delete All Appointments">
      ...
      <table>
        <tr class="headertable">
          ...
          <th>Date</th>
          <th>From</th>
          <th colspan='2' style="width:0%;padding-right:0">Action</th>
        </tr>
      <?php 
        
        $tableuser="SELECT * from appointments ORDER BY sessiondate DESC";
        ...
        while($fetchtable=mysqli_fetch_array($tableuserrun)){
            echo "<tr><td class='name'>".$fetchtable['firstname']." ".$fetchtable['familyname']."</td><td class='email1'>
            ".$fetchtable['email']."</td><td>".$fetchtable['birthday'].
            "</td><td class='phone'>".$fetchtable['phone']."</td><td class=''>".$fetchtable['doctor'].
            "</td><td>".$fetchtable['eventtime']."</td><td>".$fetchtable['righttime']."</td>
            <td class='date1'>".$fetchtable['sessiondate']."</td><td>".$fetchtable['place']."</td><td style='display:flex;margin-right:1.2vw; 
            border-bottom:none;border-left:none; margin-right:-.03vw' >
           
             <form action='app.php'><input type='submit' class='btn btn-danger deletetablebtn'
             name='delete' style='margin-left:.19vw'value='delete' id='".$fetchtable['id']."'>
             <input type='hidden' value=".$fetchtable['id']." name='id'>
             </form></td></tr>
            ";
        }
        if($fetchtable['Status'])
...
function searchdate(){
  //alert(search.value);
  var select=Number(new Date(search.value));
  //console.log(select);
  for(i=0;i<date1.length;i++){
    date1[i].style.background='none';
  var b=[];
  date1[i].style.background='none';
  b[i]=date1[i].textContent;
  var a=Number(new Date(b[i]));
  //console.log(a);
  if(a==select){
    //alert('valid');
  ...
  for(i=0;i<name1.length;i++){
    ...
    phone[i].style.background='none';
    var a=search.value;
    var b=phone[i].textContent;
    if(a.replace(/\s+/g, '') == b.replace(/\s+/g, '') ){
      //alert('valid');
      phone[i].style.backgroundColor='rgb(229 206 44)';
      phone[i].scrollIntoView();
      //window.scroll(0,findPos(date1[i]));
      body.style.scrollBehavior='smooth';
    }}
