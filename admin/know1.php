...
if(isset($_GET['useful'])){
  $id=$_GET['id']; 
  $select="UPDATE statics set useful='0'";
  mysqli_query($conn,$select);
  header('REFRESH:0;url=know1.php');
}

if(isset($_GET['notuseful'])){
  $id=$_GET['id']; 
  $select="UPDATE statics set notuseful='0'";
  mysqli_query($conn,$select);
  header('REFRESH:0;url=know1.php');
}
?>
...
<head>
  ...
  <title>
   edit img and description from knowledge
  </title>
  ...
  <li class="mazen2">
    ...
    <li>
      ...
      <li>
        ...
        <li><a href="know1.php"class="active">الصورة والوصف والافادة</a></li>
    ...
    <div class="main-panel" style="height: 100vh;text-align: right;width:82% !important;;padding:2vw">
      <!-- Navbar -->
      ...
      <?php // style="text-align:right"?>
        <form action="<?php echo $_SERVER['PHP_SELF'];?>">
          <label for="">صورة الثقافة  </label>
          <input type="file" name="img"style="text-align:right"  id="" value='<?php echo $knowimg;?>' class='form-control'>
          <label for="">وصف صورة الثقافة</label>
          <input type="text" name="text" id=""style="text-align:right" value='<?php echo $knowtext;;?>' class='form-control'>
          <input type="submit" value="Edit" name='know1' class='btn btn-success text-center'>
          </form>
        <div style="clear:both"></div>
        <hr style="padding-bottom:.5vw">
          <table>
        
        <tr class="headertable">
        
          <th>People say that the information is useful</th>
          <th style="box-sizing:unset; width:26vw;padding:0">People say that the information is not useful</th>
          <th colspan='2' style="width:10%;padding-right:.5vw">Action</th>
        </tr>
      <?php 
        
        $tableuser="SELECT * from statics";
        ...
        while($fetchtable=mysqli_fetch_array($tableuserrun)){
            ...
            echo "<tr><td style='width:23vw !important'>".$fetchtable['useful']."
            </td><td>".$fetchtable['notuseful']."</td><td style='display:flex;margin-right:1.2vw; 
            border-bottom:none;border-left:none; margin-right:-.03vw' >
            <form action='know1.php'>
            <input type='submit'style='margin:0 .2vw;width:16vw' class='btn btn-info contactable'
             name='useful'value='make number of useful 0 '>
             <input type='hidden' value=".$fetchtable['id']." name='id'>

             </form>
             <form action='know1.php'><input type='submit' class='btn btn-danger deletetablebtn'
             name='notuseful'style='margin:0 .2vw;width:16vw' value='make number of notuseful 0' id='".$fetchtable['id']."'>
             <input type='hidden' value=".$fetchtable['id']." name='id'>
             </form></td></tr>
            ";
        }
