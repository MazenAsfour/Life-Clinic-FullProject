...
if(isset($_GET['deletehealth'])){
  $id=$_GET['id'];
  $query="delete from usershealth where id='$id'";
  mysqli_query($conn,$query);
  header("REFRESH:0;url=healthuser.php");
}
?>
...
<div class="sidebar-wrapper">
  ...
  <li class="mazen2">
        ...
        <ul class="mazen" style="display:none">
          ...
          <li><a href="users.php" class="list h active"> المستحدمين</a></li>
          ...
          <li><a href="signadmin.php" class="list h">وصف الخروج والدخول</a></li>
    ...
    <div class="main-panel" style="height: 400vh;text-align: right;width:82% !important;">
      ...
      <table>
        <tr class="headertable">
          <th>Full-Name</th>
          <th>Email</th>
          <th>points of health test</th>
          <th style="box-sizing:unset; width:10vw;padding:0">health diagnosis</th>
          <th>submission date</th>
          <th colspan='2' style="width:10%;padding-right:.5vw">Action</th>
        </tr>
      <?php 
        
        $tableuser="SELECT * from usershealth";
        ...
        while($fetchtable=mysqli_fetch_array($tableuserrun)){
            ...
            ".$fetchtable['email']."</td><td >".$fetchtable['points'].
            "</td><td class='health'>".$fetchtable['health']."</td><td class='date1'>".$fetchtable['date'].
            "</td><td style='display:flex;margin-right:1.2vw; 
            border-bottom:none;border-left:none; margin-right:-.03vw' >
            <form action='users.php'>
            <input type='submit' class='btn btn-info contactable'
             name='contact'value=contact '>
             </form>
             <form action='healthuser.php'><input type='submit' class='btn btn-danger deletetablebtn'
             name='deletehealth'value='delete' id='".$fetchtable['id']."'>
...
for(i=0;i<status.length;i++){
  console.log(status[i].textContent);
  if(status[i].textContent=='active'){
    status[i].innerHTML="<span class=active1></span>active";
  }else{
    status[i].innerHTML="<span class=unactive1></span>unactive";
  }
...
body.style.scrollBehavior='smooth';
  ...
  for(i=0;i<name1.length;i++){
    ...
    health[i].style.background='none';
    var a=search.value;
    var b=health[i].textContent;
    //console.log(a,b);
    if(a==b){
      //alert('valid');
      health[i].style.backgroundColor='rgb(229 206 44)';
      health[i].scrollIntoView();
      //window.scroll(0,findPos(date1[i]));
      body.style.scrollBehavior='smooth';
    }}
  ...
  status1[i].style.background='none';
  var a=search.value;
  var b=status1[i].textContent;
  //console.log(a,a.includes("22/04/23"));
  if(b.includes(a) ){
  //alert('valid');
  //status1[i].innerHTML=b.replace(a,"<span style='background-color:red'>"+a+"</span>");
  status1[i].style.backgroundColor='rgb(229 206 44)';
  status1[i].scrollIntoView()
  //window.scroll(0,findPos(date1[i]));
  body.style.scrollBehavior='smooth';
}}*/
for(i=0;i<health.length;i++){
